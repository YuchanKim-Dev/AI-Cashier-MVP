...
class RealtimeClient:
    ...
    async def _send_session_update(self):
        system_prompt = (
            ...
            "- 장바구니에서 빼달라고 하면 remove_from_cart를 호출하세요.\n"
            "- '결제할게', '주문할게요', '그게 다야' 등의 말이 나오면 checkout을 호출하세요.\n"
            "- 장바구니가 비어있으면 checkout을 호출하지 마세요.\n"
            "\n[결제 화면]\n"
            "- 결제 화면(checkout 후)에서는 결제 수단을 안내하세요: 앱카드 또는 현장카드.\n"
            "- '앱카드', '앱으로', '앱카드로 할게' → select_payment(method='app_card') 호출.\n"
            "- '현장카드', '카드', '카드로 할게', '현장으로' → select_payment(method='physical_card') 호출.\n"
            "- 앱카드가 없다고 하면 '현장카드 단말기에 카드를 꽂아주시면 됩니다'라고 안내하세요.\n"
            "\n[공통]\n"
    ...
    async def update_instructions(self, name: str):
        ...
        system_prompt = (
            ...
            "- 장바구니가 비어있으면 checkout을 호출하지 마세요.\n"
            "\n[결제 화면]\n"
            "- 결제 화면에서는 결제 수단을 안내하세요: 앱카드 또는 현장카드.\n"
            "- '앱카드', '앱으로', '앱카드로 할게' → select_payment(method='app_card') 호출.\n"
            "- '현장카드', '카드', '카드로 할게' → select_payment(method='physical_card') 호출.\n"
            "\n[공통]\n"
            "- 응답은 2문장 이내로 짧게. 불필요한 인사말 반복 금지.\n"
            "- 가격은 항상 '원' 단위로 말하세요.\n"
